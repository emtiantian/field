...
//		}
	...
	field.prototype.init = function() {		
		var _self = this;
		this.mainEle.css({"position":"absolute","width":this.opts.mainWidth,"height":this.opts.mainHeight,"background":this.opts.background});
		//在开始初始化的时候先隐藏 或许透明更好?
		this.mainEle.hide();
		//链式调用
...
//		this.resize();
	...
	field.prototype.makeData  = function(d){
		...
		$.each(d,function(i,ele){
			...
			ele.x = _self.opts.defaultsPosition.x;
			ele.y = _self.opts.defaultsPosition.y;
...
//				hideDelay:100000,
		...
		var js = $(".windeymill-div").jsmartdrag({ 
	        target:'#main',
	        afterDrag:afterDrag
    	});
    	function afterDrag(selected,currentObj,targetSelected){ 
		    if(selected){
		       _self.debugMsg("将元素ID为"+currentObj.attr("id")+"移动到了元素ID为"+targetSelected.attr("id")+"上");    
		    }else{
		       _self.debugMsg("没有拖动到指定的区域里哦!");
		    }
	    }
		return this;
	}
	...
	field.prototype.createSpeed = function(){	
		...
		this.debugMsg(html);
		this.mainEle.append(html);
		move('.speed2').rotate(this.opts.speedData.speed).duration('2s').end();
		return this;
	}
	
	//更新方法
	field.prototype.update = function(url,timer,fun){
		var _self = this;
		$.ajax({
			type:"post",
			url:url,
...
//					_self.debugMsg(timer);
	...
	field.prototype.speedUpdate = function(data,_self){
		move('.speed2').rotate(data.speed).duration('2s').end();
	}
...
//		});
	...
	field.prototype.getPosition = function(){
		var data = []
		$.each($(".windeymill-div"),function(i,ele){
			data.push({"id":$(ele).attr("id"),"x":$(ele).position().left,"y":$(ele).position().top});
		})
		return data;
	}
	//判断是不是ios系统
